...
trait Billable
{
    ...
    public function applyAddOn($subscription = 'default')
    {
        $subscription = $this->subscription($subscription);

        if (! $subscription) {
            throw new InvalidArgumentException("Unable to apply add on. Subscription does not exist.");
        }

        $subscription->applyAddOn();
    }
